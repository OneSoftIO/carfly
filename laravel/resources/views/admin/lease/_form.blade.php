<div class="row">
    <div class="col-lg-9 col-md-9">
        ...
        <label class="alert alert-danger">Dėmesio! Jūs patvirtinote rezervaciją, bet apmokėjimo negavote arba jis yra atšauktas.</label>
        @endif
        <div class="ibox">
            ...
            <div class="ibox-content">
                ...
                <div class="row">
                    <div class="col-md-7 col-xs-12">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control datepicker" name="pickupDate" id="datetimepicker6" value="{{old('from', (isset($booking->from))?$booking->from:null)}}">
                        </div>
                    </div>
                    <div class="col-md-5 col-xs-12">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="from_hour">
                                @for($i = 0;$i < 24;$i++)
                                    <option {{(isset($booking->from_timestamp) && $booking->fromTime() == $i.":00" ?'selected':null)}} value="{{$i}}:00">{{$i}}:00</option>
                                    <option {{(isset($booking->from_timestamp) && $booking->fromTime() == $i.":30" ?'selected':null)}} value="{{$i}}:30">{{$i}}:30</option>
                                @endfor
                            </select>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-7 col-xs-12">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control datepicker" name="dropoffDate" id="datetimepicker7" value="{{old('until', (isset($booking->until))?$booking->until:null)}}">
                        </div>
                    </div>
                    <div class="col-md-5 col-xs-12">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="until_hour">
                                @for($i = 0;$i < 24;$i++)
                                    <option {{(isset($booking->until_timestamp) && $booking->untilTime() == $i.":00" ?'selected':null)}} value="{{$i}}:00">{{$i}}:00</option>
                                    <option {{(isset($booking->until_timestamp) && $booking->untilTime() == $i.":30" ?'selected':null)}} value="{{$i}}:30">{{$i}}:30</option>
                                @endfor
                            </select>
                        </div>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="info" name="info" class="form-control">{{old('info', (isset($booking->info))?$booking->info:null)}}</textarea>
                </div>
                <div class="form-group">
                    <label>Kur pristatyti automobilį? (adresas arba Jūsų skrydžio numeris)</label>
                    <input type="text" name="address" class="form-control" value="{{old('address',(isset($booking->address))?$booking->address:null)}}">
                </div>
            </div>
        </div>
            <div class="ibox">
                    ...
                    @foreach($terms as $key => $term)
                        <div class="check">
                            <input id="check{{$key}}" type="checkbox"
                                   ...
                                   @endforeach
                                   @endif
                                   name="terms[]" {{(!isset($bookingServices) && empty($term->price))?'checked':null}} value="{{$term->id}}">
                            <label for="check{{$key}}">
                                <div class="box"><i class="fa fa-check"></i></div>
                            </label>
                            <label for="check{{$key}}" class="name">{{$term->transl()->name}} | {{(!empty($term->price))?$term->price . '&euro;':'Free'}}</label>
                        </div>
                        @if($term->hasAmount())
                            <div class="form-group">
                                <div class="col-md-8">
            ...
            <div class="ibox-content">
                ...
                <div class="form-group">
                    ...
                    <select name="payment_status" class="form-control">
                        <option value="pending" {{isset($booking) && $booking->order->isPending()?'selected':null}}>Laukiama apmokėjimo</option>
                        <option value="paid" {{isset($booking) && $booking->order->isPaid()?'selected':null}}>Apmokėta</option>
                    </select>
                </div>
            </div>
        </div>
